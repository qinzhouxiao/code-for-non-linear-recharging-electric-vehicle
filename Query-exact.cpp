...
ofstream log_out("log/noPruning/test-log-all-NY-no3.txt");
ofstream exp_out("log/noPruning/test-exact-all-NY-no3.txt");
...
// tmin表示以t作为关键字搜索最短路径 0/1分别表示这条路径的时间t、需要充的电d
// dmin表示以d作为关键字搜索最短路径 0/1分别表示这条路径的时间t、需要充的电d
...
unordered_map<int, int> tmin[MAXM][2], dmin[MAXM][2];//MAXM×2个map
...
void Dijkstra(int S, unordered_map<int, int> ret[], int (*weight)(EDGE)) {
    ...
    ret[0].clear();
    ret[1].clear();
    dis[S] = dis_t[S] = dis_d[S] = 0;//dis表示到点s的最短距离为dis[s]
...
bool LabelPruning2(LABEL &label, int &ans) {
    ...
    // 2.如果到T的最短路线超过了Capacity，则无法使用这条道路进行pruning
...
void SecondStep() {
    ...
    if (TT_is_station == false) {
        for (auto st : stations) {
            ...
            // log_out << st << " => " << TT << " t=" << ans_t << " d=" << ans_d << " F=" << F(ans_t, ans_d) << ", " << ans <<  endl;
        }
    }
    while (!pq.empty()) {
        pair<int, int> pr = pq.top(); pq.pop();
...
//            clock_t t2 = clock();
            ...
            double construct_time = duration.count();
//            exp_out << "start: "<< s << "  target: " << t << "  distance: " << distance << "  construct time: " << construct_time;
            exp_out << std::fixed << std::setprecision(6) << construct_time << "  " << distance;
            exp_out << endl;
        }
